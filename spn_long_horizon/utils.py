...
class ObsBuffer:
    def __init__(self, frame_history_len=3, horizon=1):
        self.frame_history_len = frame_history_len
        self.horizon = horizon
        self.buffersize = (frame_history_len - 1) * horizon + 1
        self.last_obs_all = []

    def store_frame(self, frame):
        obs_np = frame.transpose(2,0,1)

        if len(self.last_obs_all) < self.buffersize:
            self.last_obs_all = []
            for ii in range(self.buffersize):
                self.last_obs_all.append(obs_np)
        else:
            self.last_obs_all = self.last_obs_all[1:] + [obs_np]

        return np.concatenate([self.last_obs_all[i] for i in range(0, self.buffersize, self.horizon)], 0)
...
def sample_cont_action(args, net, imgs, info=None, prev_action = None, testing=False, avg_img=0, std_img=1.0, tt=0):
    ...
    if args.sample_based_planning:
        ...
        this_action = torch.stack([torch.ones(100) * 0.0, torch.arange(100) / 50 - 1], dim=1).view(100, 1, 2).repeat(1, args.pred_step, 1)
        ...
        if not os.path.isdir('actions'):
            os.mkdir('actions')
        plt.savefig('actions/step_%04d.png' % (tt % 1e4), dpi=100)
