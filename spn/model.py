...
class ConvLSTMNet(nn.Module):
    def __init__(self, args):
        ...
        if args.use_seg:
            ...
            self.action_encode2 = nn.Linear(args.num_total_act, args.info_dim)
            self.info_encode = nn.Linear(args.info_dim + args.hidden_dim, args.hidden_dim)
            self.dla = dla.dla46x_c(pretrained = args.pretrained)
            self.info_encode2 = nn.Linear(args.info_dim + args.hidden_dim, args.hidden_dim)
            ...
            self.lstm2 = ConvLSTMCell(args.hidden_dim, args.hidden_dim, True)
    ...
    def forward(self, x, action, action2, with_encode=False, hidden=None, cell=None, hidden2=None, cell2=None, training=True):
        ...
        if self.args.use_seg and self.args.use_lstm:
            pass
        elif hidden2 is None or cell2 is None:
            hidden2 = x
            cell2 = x
        ...
        if self.args.use_seg:
            ...
            action_enc2 = F.relu(self.action_encode2(action2))
            encode = torch.cat([x, action_enc], dim = 1)
            encode2 = torch.cat([x, action_enc2], dim=1)
            encode = F.relu(self.info_encode(encode))
            encode2 = F.relu(self.info_encode2(encode2))
            hidden, cell = self.lstm(encode, [hidden, cell])
            nx_feature_enc = hidden#.view(-1, self.args.hidden_dim)
            hidden2 = self.hidden_encode(torch.cat([hidden, hidden2], dim=1))
            cell2 = self.cell_encode(torch.cat([cell, cell2], dim=1))
            hidden2, cell2 = self.lstm2(encode2, [hidden2, cell2])
            nx_feature_enc2 = hidden2
            output_dict['seg_pred'] = self.outfeature_encode(F.relu(torch.cat([nx_feature_enc, action_enc], dim = 1)))

        # major outputs
        output_dict['coll_prob'] = self.coll_layer(nx_feature_enc2, action_enc2)
        output_dict['offroad_prob'] = self.off_layer(nx_feature_enc2, action_enc2)
        output_dict['dist'] = self.dist_layer(nx_feature_enc2, action_enc2)

        # optional outputs
        if self.args.use_pos:
            output_dict['pos'] = self.pos_layer(nx_feature_enc2, action_enc2)
        if self.args.use_angle:
            output_dict['angle'] = self.angle_layer(nx_feature_enc2, action_enc2)
        if self.args.use_speed:
            output_dict['speed'] = self.speed_layer(nx_feature_enc2, action_enc2)
        if self.args.use_xyz:
            output_dict['xyz'] = self.xyz_layer(nx_feature_enc2, action_enc2)
        
        if self.args.use_seg:
            nx_feature_enc = feature_enc
        return output_dict, nx_feature_enc, hidden, cell, hidden2, cell2
...
class ConvLSTMMulti(nn.Module):
    ...
    def forward(self, imgs, actions=None, hidden=None, cell=None, get_feature=False, training=True):
        ...
        actions2 = Variable(torch.from_numpy(actions.data.cpu().numpy().copy()).cuda(), requires_grad=False)
        batch_size, num_step, c, w, h = int(imgs.size()[0]), int(imgs.size()[1]), int(imgs.size()[-3]), int(imgs.size()[-2]), int(imgs.size()[-1])
        output_dict, pred, hidden, cell, hidden2, cell2 = self.conv_lstm(imgs[:,0,:,:,:].squeeze(1), actions[:,0,:].squeeze(1), actions2[:, 0, :].squeeze(1), hidden=hidden, cell=cell, training=training)
        ...
        for i in range(1, self.args.pred_step):
            output_dict, pred, hidden, cell, hidden2, cell2 = self.conv_lstm(pred, actions[:, i, :], actions2[:, i, :], with_encode=True, hidden=hidden, cell=cell,hidden2=hidden2,cell2=cell2, training=training)
            for key in output_dict.keys():
