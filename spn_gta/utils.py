...
def sample_cont_action(args, net, imgs, info=None, prev_action=None, testing=False, avg_img=0, std_img=1.0, tt=0, action_var=None, get_first=True):
    ...
    if args.sample_based_planning:
        ...
        this_action0 = np.repeat(np.concatenate(this_action0, axis=-1).reshape(25, 1, 2), 10, axis=1)
        this_action = Variable(torch.from_numpy(this_action0).cuda().float(), requires_grad=False)
        loss = np.zeros(25)
        ...
        for i in range(4):
            ...
            if i < 3:
                this_action0 = hg.generate_samples(this_action0.reshape(25, -1), loss, 25, -1, 1).reshape(25, args.pred_step, 2)
                this_action = Variable(torch.from_numpy(this_action0).cuda().float(), requires_grad=False)
        res = hg.best_data().reshape(args.pred_step, 2)
        if get_first:
            res = res[0]
