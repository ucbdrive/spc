...
def train_policy(args, env, num_steps=40000000):
    ...
    with open(os.path.join(video_folder, 'actions.txt'), 'a') as f:
        f.write('%0.2f %s\n' % (time.time(), str(buffer_manager.prev_act)))
    ...
    for tt in range(num_imgs_start, num_steps):
        ...
        if ('gta' in args.env and done and tt - num_imgs_start > 1000) or ('gta' not in args.env and tt % args.learning_freq == 0 and tt > args.learning_starts and buffer_manager.mpc_buffer.can_sample(args.batch_size)):
        ...
        if done:
            ...
            action = np.array([-1.0, 0.0]) if args.continuous else 1
            with open(os.path.join(video_folder, 'actions.txt'), 'a') as f:
                f.write('%0.2f %s\n' % (time.time(), str(action)))
