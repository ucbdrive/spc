...
from baselines.ddpg_speed.ddpg import DDPG
...
def train(env, nb_epochs, nb_epoch_cycles, render_eval, reward_scale, render, param_noise, actor, critic,
    ...
    tau=0.01, eval_env=None, param_noise_adaption_interval=50, env_id=0):
        ...
        for epoch in range(nb_epochs):
            for cycle in range(nb_epoch_cycles):
                ...
                for t_rollout in range(nb_rollout_steps):
                    # Predict next action.
                    action, q = agent.pi(obs, np.array([env.target_speed]), apply_noise=True, compute_Q=True)
                    ...
                    with open('/tmp/ddpg_speed_log_%d.txt' % env_id, "a") as f:
                        f.write('target %0.4f speed %0.4f\n' % (env.target_speed, info['speed']))
                    t += 1
                    if rank == 0 and render:
                        env.render()
                    episode_reward += r
                    ...
                    epoch_qs.append(q)
                    agent.store_transition(obs, action, r, new_obs, done, env.target_speed)
                    obs = new_obs
                    ...
                    if done:
                        ...
                        if done_cnt % 5 == 0:
                            ...
                            with open('/tmp/ddpg_eval_log_%d.txt' % env_id, 'a') as f:
                                f.write('step %d reward %0.4f\n' % (t, eval_episode_reward))
                            obs = env.reset()
