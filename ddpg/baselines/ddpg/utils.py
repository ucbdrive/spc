...
def get_action_loss(args, net, imgs, actions, target = None, hidden = None, cell = None, gpu = 0):
    ...
    loss = 0
    if args.sample_with_collision:
        # coll_ls = output['coll_prob'][:, :, 1]
        coll_ls = Focal_Loss(output['coll_prob'].view(-1, 2), target['coll_batch'].view(-1).long(), reduce=not args.sample_based_planning)
        ...
        loss += coll_ls
    if args.sample_with_offroad:
        # off_ls = output['offroad_prob'][:, :, 1]
        off_ls = Focal_Loss(output['offroad_prob'].view(-1, 2), target['off_batch'].view(-1).long(), reduce=not args.sample_based_planning)
