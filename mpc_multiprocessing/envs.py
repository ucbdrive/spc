...
def create_env(env_id, verbose=False, reward_ben=False, config='quickrace_discrete_single.xml', rescale=True, continuous=False):
    if env_id == 'gridworld-v0':
        ...
        env = torcs_env(game_config = '/home/cxy/pyTORCS/game_config/' + config)
    elif env_id == 'torcs-v2':
        from py_TORCS import torcs_envs
        env = torcs_envstorcs_envs(num = 1, isServer = 1, game_config = '/home/cxy/pyTORCS/game_config/' + config).get_envs()[0]
    else:
        env = gym.make(env_id)
        env = AtariRescale42x42(env)
    return env
