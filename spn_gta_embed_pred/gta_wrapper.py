...
class GTAWrapper:
    def __init__(self, env, imsize=(256, 256), random_reset=True, continuous=True):
        ...
        self.num_off = 0
        self.obs = None
        self.model = dla_up.__dict__.get('dla60up')(32, None, down_ratio=2)
        self.model = torch.nn.DataParallel(self.model.cuda())
        self.model.load_state_dict(torch.load('model_best1.pth.tar')['state_dict'])

    def get_segmentation(self):
    	# obs = Variable(torch.from_numpy(np.transpose(self.obs, (2, 0, 1)))).cuda().unsqueeze(0).float()
        #print('get segmentation')
        seg = self.model(self.obs)[1]
