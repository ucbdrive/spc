...
def train_model(args, train_net, mpc_buffer, epoch, avg_img_t, std_img_t):
    ...
    if args.use_collision:
        ...
        for i in range(args.pred_step):
            colls.append(show_accuracy(coll_target_np[:, i], coll_np[:, i], 'Step %d collision' % (i+1)))
    ...
    if args.use_offroad:
        ...
        offs = []
        for i in range(args.pred_step):
            offs.append(show_accuracy(off_target_np[:, i], off_np[:, i], 'Step %d offroad' % (i+1)))
...
def sample_cont_action(args, p, net, imgs, guides, info=None, prev_action=None, testing=False, avg_img=0, std_img=1.0, tt=0, action_var=None):
    ...
    if args.sample_based_planning:
        ...
        # this_action1 = np.array([-1.0, -0.2, 0.0, 0.2, 1.0])
        ...
        # this_action = torch.from_numpy(this_action0).view(30, 1, 2).repeat(1, args.pred_step, 1)
