...
class convLSTM3(nn.Module):
    def __init__(self):
        ...
        self.lstm0 = convLSTMCell3(65, 64, kernel_size=5, padding=2)
        self.lstm1 = convLSTMCell3(65, 64, kernel_size=3, padding=1)
        self.lstm2 = convLSTMCell3(129, 128, kernel_size=3, padding=1)
        self.lstm3 = convLSTMCell3(257, 256, kernel_size=1, padding=0)

    def forward(self, x):
        batch_size = x[0][0].shape[0]

        hx0 = Variable(torch.zeros(batch_size, 64, 64, 32))
        cx0 = Variable(torch.zeros(batch_size, 64, 64, 32))
        hx1 = Variable(torch.zeros(batch_size, 64, 32, 16))
        cx1 = Variable(torch.zeros(batch_size, 64, 32, 16))
        hx2 = Variable(torch.zeros(batch_size, 128, 16, 8))
        cx2 = Variable(torch.zeros(batch_size, 128, 16, 8))
        hx3 = Variable(torch.zeros(batch_size, 256, 8, 4))
        cx3 = Variable(torch.zeros(batch_size, 256, 8, 4))
