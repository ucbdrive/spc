...
class MPCBuffer(object):
    ...
    def can_sample_guide(self, batch_size):
        return self.num_in_buffer - 2 * np.sum(self.done) >= batch_size

    def sample_guide(self, batch_size):
        indices = []
        while len(indices) < batch_size:
            idx = random.randint(self.num_in_buffer-1)
            if idx not in indices and np.sum(self.done[idx:idx+1]) == 0:
                indices.append(idx)

        data = dict()
        data['obs'] = Variable(torch.from_numpy(np.concatenate([self.obs[idx][np.newaxis, :] for idx in indices], axis=0)).float() / 255.0, requires_grad=False)
        data['next_obs'] = Variable(torch.from_numpy(np.concatenate([self.obs[idx+1][np.newaxis, :] for idx in indices], axis=0)).float() / 255.0, requires_grad=False)
        data['action'] = Variable(torch.from_numpy(self.guide_action[indices]), requires_grad=False)
        data['reward'] = Variable(torch.from_numpy([self.reward[indices]]))
