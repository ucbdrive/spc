...
class PRED(nn.Module):
    def __init__(self):
        ...
        self.action_encoder1 = nn.Linear(6, 800)
        self.action_encoder2 = nn.Linear(6, 800)
    ...
    def forward(self, x, action):
        ...
        y1 = F.sigmoid(self.action_encoder1(one_hot).view(8, 4, 5, 5))
        y2 = F.sigmoid(self.action_encoder2(one_hot).view(4, 8, 5, 5))
        result = F.relu(F.conv2d(x, y1, stride = 1, padding = 2))
        result = F.conv2d(result, y2, stride = 1, padding = 2)
        return result
...
if __name__ == '__main__':
    if os.path.exists('separate_conv_log.txt'):
        os.system('rm separate_conv_log.txt')
    if not os.path.isdir('separate_conv_models'):
        os.mkdir('separate_conv_models')
    np.random.seed(0)
    ...
    for ooo in range(100):
        for idx in idx_list:
            ...
            if cnt % 100 == 0:
                torch.save(model.state_dict(), os.path.join('separate_conv_models', 'epoch%d.dat' % cnt))
