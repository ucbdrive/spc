...
if __name__ == '__main__':
    ...
    if train:
        inputs = torch.autograd.Variable(torch.ones(13, 9, 480, 640), requires_grad = False)
        target = torch.autograd.Variable(torch.ones(13, 480, 640), requires_grad = False).type(torch.LongTensor)
        target_pos = torch.autograd.Variable(torch.zeros(13), requires_grad = False)
        target_angle = torch.autograd.Variable(torch.zeros(13), requires_grad = False)
        target_speed = torch.autograd.Variable(torch.zeros(13), requires_grad = False)
        NLL = nn.NLLLoss2d()
        BCE = nn.BCELoss()
        L1 = nn.SmoothL1Loss()
        L2 = nn.MSELoss()
    ...
    if train:
        ...
        while True: # while
            ...
            for i in range(600): # 2000
                ...
                inputs[:] = torch.from_numpy(data['obs'])
                action = data['action']
                target[:] = torch.from_numpy(data['seg']).type(torch.LongTensor)
                target_pos[:] = torch.from_numpy(data['pos'])
                target_angle[:] = torch.from_numpy(data['angle'])
                target_speed[:] = torch.from_numpy(data['speed'])
                output, feature_map = model(inputs[0].view(1, 9, 480, 640))
                pos, angle, speed = further(feature_map)
                loss0 = 0 # NLL(output, target[0].view(1, 480, 640))
                loss1 = 0
                loss2 = L2(pos, target_pos[0])
                loss3 = L2(angle, target_angle[0]) * 4
                loss4 = L2(speed, target_speed[0]) / 10
                loss = loss2 + loss3 + loss4
                # LOSS[0, 0] += loss0.data.cpu().numpy()[0]
                # LOSS[0, 1] += loss1.data.cpu().numpy()[0]
                LOSS[0, 2] += loss2.data.cpu().numpy()[0]
                LOSS[0, 3] += loss3.data.cpu().numpy()[0]
                LOSS[0, 4] += loss4.data.cpu().numpy()[0]
                gamma = 1
                for j in range(1, num_steps + 1):
                    ...
                    loss0 = 0 # NLL(output, target[j].view(1, 480, 640))
                    loss1 = L2(feature_map, target_feature_map) / 1000
                    loss2 = L2(pos, target_pos[j])
                    loss3 = L2(angle, target_angle[j]) * 4
                    loss4 = L2(speed, target_speed[j]) / 10
                    loss += gamma * (loss1 + loss2 + loss3 + loss4)
                    # LOSS[j, 0] += loss0.data.cpu().numpy()[0]
                    LOSS[j, 1] += loss1.data.cpu().numpy()[0]
                    LOSS[j, 2] += loss2.data.cpu().numpy()[0]
                    LOSS[j, 3] += loss3.data.cpu().numpy()[0]
                    LOSS[j, 4] += loss4.data.cpu().numpy()[0]
