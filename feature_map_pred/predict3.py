    ...
    if train:
        ...
        while True: # while
            ...
            for i in range(2000): # 2000
                ...
                loss = NLL(output, target[0].view(1, 480, 640)) + L2(pos, target_pos[0]) + L2(angle, target_angle[0]) + L2(speed, target_speed[0])
                LOSS[0, 0] += NLL(output, target[0].view(1, 480, 640)).data.cpu().numpy()[0]
                LOSS[0, 1] += L2(pos, target_pos[0]).data.cpu().numpy()[0]
                LOSS[0, 2] += L2(angle, target_angle[0]).data.cpu().numpy()[0]
                LOSS[0, 3] += L2(speed, target_speed[0]).data.cpu().numpy()[0]
                gamma = 1
                for j in range(1, num_steps + 1):
                    ...
                    loss += gamma * (NLL(output, target[j].view(1, 480, 640)) + L2(pos, target_pos[j]) + L2(angle, target_angle[j]) + L2(speed, target_speed[j]))
                    LOSS[j, 0] += NLL(output, target[j].view(1, 480, 640)).data.cpu().numpy()[0]
                    LOSS[j, 1] += L2(pos, target_pos[j]).data.cpu().numpy()[0]
                    LOSS[j, 2] += L2(angle, target_angle[j]).data.cpu().numpy()[0]
                    LOSS[j, 3] += L1(speed, target_speed[j]).data.cpu().numpy()[0]
