...
class PRED(nn.Module):
    def __init__(self):
        ...
        self.lstm = nn.LSTMCell(128, 400)

    def forward(self, x, action, hx, cx):
        one_hot = torch.zeros(1, 6)
        one_hot[0, action] = 1
        one_hot = Variable(one_hot, requires_grad = False)
        if torch.cuda.is_available():
            one_hot = one_hot.cuda()
        y = self.action_encoder(one_hot)
        hx, cx = self.lstm(y, (hx, cx))
        y = hx.view(4, 4, 5, 5)
        result = F.conv2d(x, y, stride = 1, padding = 2)
        return result, hx, cx
...
if __name__ == '__main__':
    if os.path.exists('lstm_conv_log.txt'):
        os.system('rm lstm_conv_log.txt')
    if not os.path.isdir('lstm_conv_models'):
        os.mkdir('lstm_conv_models')
    np.random.seed(0)
    ...
    for ooo in range(100):
        for idx in idx_list:
            ...
            if cnt % 100 == 0:
                torch.save(model.state_dict(), os.path.join('lstm_conv_models', 'epoch%d.dat' % cnt))
