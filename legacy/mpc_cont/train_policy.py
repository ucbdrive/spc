...
def train_policy(args,
    ...
    train_net = ConvLSTMMulti(3,3,num_total_act, True, multi_info=False, with_posinfo=with_posinfo, use_pos_class=use_pos_class, with_speed=with_speed, with_pos=with_pos, frame_history_len=frame_history_len)
    if use_cuda:
    ...
    train_net, epoch, optimizer = load_model(args.save_path+'/model', train_net, data_parallel = True, optimizer=optimizer)
    train_net.train()
    ...
    for tt in range(num_imgs_start, num_steps):
        ...
        if rand_num <= 1-explore:
            with torch.no_grad():
                train_net.eval()
                action = sample_action(net, obs_var, speed=sp_var, pos=pos_var, posxyz=posxyz_var, num_time=12, batch_step=350, hand=False)
        else:
        ...
        prev_act = action
        done = doneCond.isdone(info['trackPos'], dist_this) or epi_len > 1000
        ...
        if tt % args.target_update_freq == 0:
            if mpc_buffer.can_sample(batch_size):
                with torch.no_grad():
                    train_net.eval()
                    net_loss, net_coll_acc, net_off_acc, net_dist_ls = train_model(train_net, mpc_buffer, batch_size, epoch, avg_img_t, std_img_t, pred_step)
        if tt % 100 == 0:
            avg_img, std_img, avg_img_t, std_img_t = img_buffer.get_avg_std(gpu=0)
