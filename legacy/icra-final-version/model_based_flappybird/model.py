...
class ConvLSTMNet(nn.Module):
    def __init__(self, args):
        ...
        # output layers
        if args.one_hot:
            self.reward_layer = end_layer(args, args.classes, 1)
            self.value_layer = end_layer(args, args.classes, 1)
        else:
            self.reward_layer = end_layer(args, 1, 1)
            self.value_layer = end_layer(args, args.frame_history_len, 1)
    ...
    def forward(self, x, action, with_encode=False, hidden=None, cell=None, training=True, action_var=None):
        ...
        output_dict['reward'] = self.reward_layer(rx.detach())
        output_dict['value'] = self.value_layer(rx.detach())
    ...
    def get_feature(self, x, next_obs=False):
        ...
        if next_obs:
            if self.args.use_seg:
                ...
                x = x.view(batch_size * pred_step, 3 * frame_history_len, frame_height, frame_width)
                x = self.conv_lstm.get_feature(x)
                x = x.contiguous().view(batch_size, pred_step, -1)
            return x
        else:
