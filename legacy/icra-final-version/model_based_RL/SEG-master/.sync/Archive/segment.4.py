        ...
        if i % print_freq == 0:
            print('Test: [{0}/{1}]\t'
                  ...
                  'Score {score:.2f}'.format(
                      ...
                      score=round(np.nanmean(per_class_iu(hist)) * 100, 2)), flush=True)

    print(
        ' * Score {score:.2f}'.format(score=round(np.nanmean(per_class_iu(hist)) * 100, 2)))
...
def train_seg(args):
    ...
    if args.cr and args.se:
        import sedla_up_cr as dla_up
    elif args.se and args.ps:
...
def parse_args():
    ...
    parser.add_argument('--lk', action='store_true', default=False)
