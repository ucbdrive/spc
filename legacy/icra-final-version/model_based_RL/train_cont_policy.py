    ...
    def store_frame(self, obs, info, seg):
        ...
        if 'torcs' in self.args.env:
            ...
            if self.args.use_xyz:
                ...
                rela_xyz = None
            self.rewards.append(info['speed'] * info['angle'])
            ...
            self.rewards.append(info['speed'])
            self.reward_idx.append(self.mpc_ret)
            self.mpc_buffer.store_effect(idx=self.mpc_ret,
                                         coll=info['coll_flag'],
                                         off=info['off_flag'],
    ...
    def store_effect(self, action, reward, done):
        ...
        if done:
            for i in range(len(self.rewards)-1, 0, -1):
                self.rewards[i-1] += self.args.gamma * self.rewards[i]
            for i in range(len(self.rewards)):
                self.mpc_buffer.value[self.reward_idx[i], 0] = self.rewards[i]
            self.rewards = []
            self.reward_idx = []
...
def train_policy(args, env, num_steps=40000000):
    ...
    for tt in range(num_imgs_start, num_steps):
        ...
        ret, obs_var = buffer_manager.store_frame(obs, info, None)
        if args.normalize:
        ...
        if done and tt > 300 and buffer_manager.mpc_buffer.can_sample(args.batch_size):
            # train_model_new(args, train_net, buffer_manager.mpc_buffer, tt)
            for ep in range(20):  # range(max(int((tt - last_tt) * args.num_train_steps / 100.0), args.num_train_steps)):
                optimizer.zero_grad()
