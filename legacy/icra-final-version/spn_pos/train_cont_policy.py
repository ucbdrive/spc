...
class BufferManager:
    def __init__(self, args=None):
        ...
        if args.resume:
            pass#self.mpc_buffer.load(args.save_path)
    ...
    def save_mpc_buffer(self):
        return #self.mpc_buffer.save(self.args.save_path)

    def load_mpc_buffer(self):
        return #self.mpc_buffer.load(self.args.save_path)
...
def train_policy(args, env, num_steps=40000000):
    ...
    if 'torcs' in args.env:
        env = TorcsWrapper(env, imsize=(args.frame_height, args.frame_width), random_reset=args.use_random_reset, continuous=args.continuous)
    ...
    for tt in range(num_imgs_start, num_steps):
        if 'torcs' in args.env:
            seg = env.env.get_segmentation().reshape((1, args.frame_height, args.frame_width)) if args.use_seg else None
        if 'gta' in args.env:
        ...
        if args.target_pos != 0:
            with open(os.path.join(args.save_path, 'poslog.txt'), 'a') as f:
                f.write('step %d pos %0.4f target %0.4f\n' % (tt, info['trackPos'], args.target_pos))
        ...
        if ('gta' in args.env and done and tt > 1000) or ('gta' not in args.env and tt % args.learning_freq == 0 and tt > args.learning_starts and buffer_manager.mpc_buffer.can_sample(args.batch_size)):
            ...
            if 'gta' in args.env:
                ...
                if args.target_pos != 0:
                    args.target_pos = np.random.uniform(-3, 3)
        ...
        if done and 'gta' not in args.env:
            ...
            if args.target_pos != 0:
                args.target_pos = np.random.uniform(2, 5) * np.random.choice([-1, 1])
