...
def plot_reward():
    ...
    read('ddpg_rgb.txt', 3)
    read('ddpg_seg.txt', 3)
    read('model_based_rl.txt', 7)
    read('mpc_only.txt', 7)
    read('mpc_seg.txt', 7)
    read('mpc_EM_optimize.txt', 7)

    plt.xlabel('steps')
    plt.ylabel('reward')
    plt.legend(['DDPG on RGB images',
                'DDPG on pretrained segmentation',
                'Model based RL',
                'MPC on pretrained segmentation',
                'MPC trained end-to-end with segmentation',
                'MPC with EM Optimization'])
    plt.tight_layout()
