...
class uncertain_buffer(object):
    def __init__(self, args):
        ...
        self.args = args
        self.predict_length = int(args.uncertain_xyz) * 2 + int(args.uncertain_action) * 2 + int(args.uncertain_pos) + int(args.uncertain_angle) + int(args.uncertain_speed)
    ...
    def uncertainty(self, xyz=None, action=None, pos=None, angle=None, speed=None):
        ...
        joint_log_likelihood = self.joint_model.score(joint)
        condition_log_likelihood = self.condition_model.score(condition)

        return np.exp(joint_log_likelihood - condition_log_likelihood)