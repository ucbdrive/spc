...
def model_forward(args, pred_model, seg_model, obs_batch, action_batch, action_var):
    batch_size, steps, channels, height, width = obs_batch.size()
    features = pred_model(
        imgs=obs_batch,
        actions=action_batch,
        phase=1,
        action_var=action_var
    ).view(batch_size*(args.frame_history_len+args.pred_step), args.classes, height, width)
    
    softmax, log_softmax = seg_model(features, mode='up')
    softmax = softmax.detach().contiguous().view(batch_size, args.frame_history_len+args.pred_step, args.classes, args.frame_height, args.frame_width)
    softmax_stack = torch.stack([softmax[:, i+1: args.frame_history_len+i+1, :, :, :].contiguous().view(batch_size, args.frame_history_len*args.classes, args.frame_height, args.frame_width) for i in range(args.pred_step)], dim=1).view(batch_size*args.pred_step, args.frame_history_len*args.classes, args.frame_height, args.frame_width)
    softmax = softmax[:, args.frame_history_len:, :, :, :].contiguous().view(batch_size*args.pred_step, args.classes, args.frame_height, args.frame_width)
    
    output_dict = pred_model(
        phase=2,
        softmax=softmax,
        softmax_stack=softmax_stack
    )
    output_dict['coll_prob'] = output_dict['coll_prob'].contiguous().view(batch_size, args.pred_step, 2)
    output_dict['offroad_prob'] = output_dict['offroad_prob'].contiguous().view(batch_size, args.pred_step, 2)
    output_dict['dist'] = output_dict['dist'].contiguous().view(batch_size, args.pred_step, 1)
    output_dict['seg_pred'] = log_softmax.contiguous().view(batch_size, args.frame_history_len+args.pred_step, args.classes, args.frame_height, args.frame_width)[:, args.frame_history_len:, :, :, :]
    return output_dict
...
def train_model(args, train_net, seg_model, mpc_buffer, epoch, avg_img_t, std_img_t):
    if epoch % 20 == 0:
    ...
    if args.use_seg:
        ...
        with torch.no_grad():
            nximg_enc = train_net(target['nx_obs_batch'], get_feature=True, next_obs=True).detach()
    ...
    output = model_forward(args, train_net, seg_model, target['obs_batch'], target['act_batch'], action_var=target['prev_action'])
    ...
    if args.use_seg:
        output['seg_pred'] = output['seg_pred'].contiguous().view(args.batch_size * args.pred_step, args.classes, args.frame_height, args.frame_width)
        target['seg_batch'] = target['seg_batch'].contiguous().view(args.batch_size * args.pred_step, args.frame_height, args.frame_width)
...
def draw_from_pred_torcs(pred):
    illustration = np.zeros((pred.shape[0], pred.shape[1], 3)).astype(np.uint8)
    illustration[:, :, 0] = 255
...
def visualize(args, target, output):
    ...
    if args.use_seg:
        ...
        target['seg_batch'] = target['seg_batch'].view(args.batch_size, args.pred_step, args.frame_height, args.frame_width)
        segmentation = from_variable_to_numpy(target['seg_batch'][batch_id])
        output['seg_pred'] = output['seg_pred'].view(args.batch_size, args.pred_step, args.classes, args.frame_height, args.frame_width)
        ...
        for i in range(args.pred_step):
            imsave('visualize/%d.png' % i, np.concatenate([draw_from_pred(segmentation[i]), draw_from_pred(prediction[i])], 1))
    ...
    def store_frame(self, frame):
        if len(self.last_obs_all) < self.frame_history_len:
            ...
            for ii in range(self.frame_history_len):
                self.last_obs_all.append(frame)
        else:
            self.last_obs_all = self.last_obs_all[1:] + [frame]
...
def sample_cont_action(args, net, seg_model, imgs, info=None, prev_action=None, testing=False, avg_img=0, std_img=1.0, tt=0, action_var=None):
    ...
    if args.sample_based_planning:
        ...
        this_action0 = np.arange(5) / 2.5 - 1.0
        this_action1 = np.arange(5) / 5 - 0.5
        ...
        this_action = torch.from_numpy(this_action0).view(25, 1, 2).repeat(1, args.pred_step, 1)
        this_action = Variable(this_action.cuda().float(), requires_grad=False)
        loss = np.zeros(25)
        with torch.no_grad():
            start_time = time.time()
            loss = get_action_loss(args, net, seg_model, imgs, this_action, action_var, None, None, None).data.cpu().numpy()
        ...
        if False:
            ...
            plt.close()
        idx = np.argmin(loss)
        res = this_action0.reshape((25, 2))[idx, :]
        return res
...
def get_action_loss(args, net, seg_model, imgs, actions, action_var=None, target=None, hidden=None, cell=None, gpu=0):
    ...
    output = model_forward(args, net, seg_model, imgs, actions, action_var=action_var)

    loss = 0

    if args.sample_with_collision:
        output['coll_prob'] = F.softmax(output['coll_prob'], dim=-1)
        if 'torcs' in args.env:
            coll_ls = -torch.round(output['coll_prob'][:, :, 0]) * output['dist'].view(-1, args.pred_step) + torch.round(output['coll_prob'][:, :, 1]) * 20.0
        elif 'carla' in args.env or 'gta' in args.env:
    ...
    if args.sample_with_offroad:
        output['offroad_prob'] = F.softmax(output['offroad_prob'], dim=-1)
        if 'torcs' in args.env:
            ...
            off_ls = -torch.round(output['offroad_prob'][:, :, 0]) * output['speed'].view(-1, args.pred_step) + torch.round(output['offroad_prob'][:, :, 1]) * 15.0
        if args.sample_based_planning:
