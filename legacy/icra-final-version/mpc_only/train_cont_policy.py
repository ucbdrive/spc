...
class ActionSampleManager:
    ...
    def sample_action(self, net, seg_model, dqn_net, obs, obs_var, action_var, exploration, tt, avg_img, std_img, info, no_explore=False, must_explore=False):
        if tt % self.args.num_same_step != 0:
            ...
            if self.args.continuous:
                if (random.random() <= 1 - exploration.value(tt) or no_explore) and not must_explore:
                    action = sample_cont_action(self.args, net, seg_model, obs_var, info=info, prev_action=np.array([0.5, 0.01]), avg_img=avg_img, std_img=std_img, tt=tt, action_var=action_var)
                else:
...
def train_policy(args, env, num_steps=40000000):
    seg_model = DLASeg(args)
    for param in seg_model.parameters():
        param.requires_grad = False
    seg_model.eval()
    if torch.cuda.is_available():
        seg_model = seg_model.cuda()
    seg_model.load_state_dict(torch.load('dlaseg.pth'))
    if args.data_parallel:
        seg_model = torch.nn.DataParallel(seg_model)

    ''' basics '''
    if 'torcs' in args.env:
        env = TorcsWrapper(seg_model, env, imsize=(args.frame_height, args.frame_width), random_reset=args.use_random_reset, continuous=args.continuous)
    ...
    done_cnt = 0
    _, info = env.reset()
    (obs, seg), reward, done, info = env.step(buffer_manager.prev_act)
    ...
    for tt in range(num_imgs_start, num_steps):
        if 'gta' in args.env:
            seg = env.get_segmentation().reshape((1, args.frame_height, args.frame_width)) if args.use_seg else None
        ...
        action, dqn_action = action_manager.sample_action(net, seg_model, dqn_agent, obs, obs_var, action_var, exploration, tt, avg_img, std_img, info, no_explore=no_explore, must_explore=must_explore)
        ...
        if 'torcs' in args.env:
            ...
            print('action', "{0:.2f}".format(action[0]), "{0:.2f}".format(action[1]),
                  ...
                  ' offroad ', str(bool(info['off_flag'])),
                  ' speed ', "{0:.2f}".format(info['speed']),
                  ' reward_with_pos ', "{0:.2f}".format(reward['with_pos']),
                  ' reward_without_pos ', "{0:.2f}".format(reward['without_pos']),
                  ' explore ', "{0:.2f}".format(exploration.value(tt)))
        ...
        if ('gta' in args.env and done and tt > 1000) or ('gta' not in args.env and tt % args.learning_freq == 0 and tt > args.learning_starts and buffer_manager.mpc_buffer.can_sample(args.batch_size)):
            # train_model_new(args, train_net, buffer_manager.mpc_buffer, tt)
            for ep in range(args.num_train_steps):
                optimizer.zero_grad()
                loss = train_model(args, train_net, seg_model, buffer_manager.mpc_buffer, epoch, buffer_manager.avg_img, buffer_manager.std_img)
                print('loss = %0.4f\n' % loss.data.cpu().numpy())
                loss.backward()
                optimizer.step()
                epoch += 1
        ...
        if done and 'gta' not in args.env:
            ...
            action_var = Variable(torch.from_numpy(np.array([-1.0, 0.0])).repeat(1, args.frame_history_len - 1, 1), requires_grad=False)
            if 'torcs' in args.env:
                (obs, seg), prev_info = env.reset(restart=True)
            elif 'carla' in args.env:
                obs, prev_info = env.reset(testing=no_explore)
            elif 'gta' in args.env:
                obs, prev_info = env.reset()
                # print('reset!!! ')
            (obs, seg), _, _, info = env.step(np.array([-1.0, 0.0])) if args.continuous else env.step(1)
