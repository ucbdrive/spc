...
def train_model(args, train_net, mpc_buffer, epoch, avg_img_t, std_img_t):
    ...
    if args.use_seg:
        output['seg_pred'] = output['seg_pred'].contiguous().view(args.batch_size * (args.pred_step + 1), args.classes, args.frame_height, args.frame_width)
        target['seg_batch'] = target['seg_batch'].contiguous().view(args.batch_size * (args.pred_step + 1), args.frame_height, args.frame_width)
...
def visualize(args, target, output):
    ...
    if args.use_seg:
        ...
        for i in range(args.pred_step):
            imsave('visualize/%d.png' % i, np.concatenate([cv2.cvtColor(observation[i], cv2.COLOR_BGR2RGB), draw_from_pred(segmentation[i]), draw_from_pred(prediction[i])], 1))
...
def get_action_loss(args, net, imgs, actions, action_var=None, target=None, hidden=None, cell=None, gpu=0):
    ...
    if args.sample_with_collision:
        ...
        if 'torcs' in args.env:
            coll_ls = -torch.round(output['coll_prob'][:, :, 0]) + torch.round(output['coll_prob'][:, :, 1])
    ...
    if args.sample_with_offroad:
        ...
        if 'torcs' in args.env:
            off_ls = -torch.round(output['offroad_prob'][:, :, 0]) + torch.round(output['offroad_prob'][:, :, 1])
        elif 'carla' in args.env:
    ...
    if args.target_speed > 0:
        ...
        loss = speed_loss
        coll_ls = (-torch.round(output['coll_prob'][:, :, 0]).view(-1, args.pred_step, 1) * weight).sum(-1).sum(-1)
        off_ls = (-torch.round(output['offroad_prob'][:, :, 0]).view(-1, args.pred_step, 1) * weight).sum(-1).sum(-1)
        loss += coll_ls
        loss += off_ls
