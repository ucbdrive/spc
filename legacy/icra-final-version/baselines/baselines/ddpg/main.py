...
def run(env_id, seed, noise_type, layer_norm, evaluation, **kwargs):
    ...
    if env_id == 'TORCS-v0':
        env = gym.make(env_id)
        env.init(isServer=1, continuous=True, resize=True, ID=-1)
    ...
    if evaluation and rank==0:
        if env_id == 'TORCS-v0':
            ...
            eval_env.init(isServer=1, continuous=True, resize=True, ID=-2)
            eval_env = TorcsWrapper(env, random_reset=False, continuous = True)
        else:        
            eval_env = gym.make(env_id)
    else:
        eval_env = None
