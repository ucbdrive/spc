...
def sample_cont_action(args, net, imgs, info=None, prev_action=None, testing=False, avg_img=0, std_img=1.0, tt=0, action_var=None):
    ...
    if args.sample_based_planning:
        hg = hyper_GMM(k=4, criterion='aic')
        ...
        for i in range(4):
            with torch.no_grad():
                loss = get_action_loss(args, net, imgs, this_action, action_var, None, None, None).data.cpu().numpy().reshape(-1)
            if i < 3:
                this_action0 = hg.generate_samples(this_action0.reshape(25, -1), loss, 25, -1, 1).reshape(25, 10, 2)
                this_action = Variable(torch.from_numpy(this_action0).cuda().float(), requires_grad=False)
        res = hg.best_data().reshape(10, 2)[0]
