...
class IDAUp(nn.Module):
    ...
    def __init__(self, large_kernel, small_kernel, out_dim, channels, up_factors):
        ...
        for i in range(1, len(channels)):
            ...
            setattr(self, 'node_LKGC_' + str(i), GlobalConvolution(out_dim, out_dim, large_kernel, small_kernel))
    ...
    def forward(self, layers):
        ...
        for i in range(1, len(layers)):
            ...
            LKGC = getattr(self, 'node_LKGC_' + str(i))
            x = LKGC(RRBR(CAB(RRBL(x), layers[i])))
            y.append(x)
        return x, y
...
def sedla34up_lkcr(classes, pretrained_base=None, **kwargs):
