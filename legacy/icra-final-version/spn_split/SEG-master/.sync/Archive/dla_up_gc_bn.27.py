    ...
    def __init__(self, channels, scales=(1, 2, 4, 8, 16), in_channels=None, version='v1'):
        ...
        for i in range(len(channels) - 1):
            ...
            if version == 'v1':
                ...
                setattr(self, 'ida_{}'.format(i),
                        IDAUpV3(15, 3, channels[j], in_channels[j:],
                              scales[j:] // scales[j]))
            elif version == 'v4':
                setattr(self, 'ida_{}'.format(i),
                        IDAUpV4(15, 3, channels[j], in_channels[j:],
                              scales[j:] // scales[j]))
            # elif version == 'v3':
            #     setattr(self, 'ida_{}'.format(i),
            #             IDAUpV1(21, 3, channels[j], in_channels[j:],
            #                   scales[j:] // scales[j]))
            # elif version == 'v4':
            #     setattr(self, 'ida_{}'.format(i),
            #             IDAUpV2(21, 3, channels[j], in_channels[j:],
            #                   scales[j:] // scales[j]))
            scales[j + 1:] = scales[j]
...
def dla34up_gc_bn_v1_21(classes, pretrained_base=None, **kwargs):
    return DLASeg('dla34', classes, pretrained_base=pretrained_base, version='v1_21', **kwargs)

def dla34up_gc_bn_v2_21(classes, pretrained_base=None, **kwargs):
    return DLASeg('dla34', classes, pretrained_base=pretrained_base, version='v2_21', **kwargs)
