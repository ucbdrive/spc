...
class BufferManager:
    ...
    def store_effect(self, action, reward, done):
        ...
        self.mpc_buffer.store_action(self.mpc_ret, action, done, np.exp(self.rewards_without))
        ...
        if tt % args.learning_freq == 0 and tt > args.learning_starts and buffer_manager.mpc_buffer.can_sample(args.batch_size):
            for ep in range(args.num_train_steps):
                ...
                if epoch % args.save_freq == 0:
                    torch.save(train_net.module.state_dict() if args.data_parallel else train_net.state_dict(), args.save_path+'/model/pred_model_'+str(tt).zfill(9)+'.pt')
