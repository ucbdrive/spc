...
class PRED(nn.Module):
    ...
    def forward(self, x, action):
        ...
        y = F.sigmoid(self.action_encoder1(one_hot).view(4, 4, 5, 5))
        result = F.conv2d(x, y, stride = 1, padding = 2) + F.conv2d(x, y, stride = 1, padding = 4, dilation = 2)
        return result
...
if __name__ == '__main__':
    if os.path.exists('dilate_and_conv_log.txt'):
        os.system('rm dilate_and_conv_log.txt')
    if not os.path.isdir('dilate_and_conv_models'):
        os.mkdir('dilate_and_conv_models')
    np.random.seed(0)
    ...
    for ooo in range(100):
        for idx in idx_list:
            ...
            if cnt % 100 == 0:
                torch.save(model.state_dict(), os.path.join('dilate_and_conv_models', 'epoch%d.dat' % cnt))
