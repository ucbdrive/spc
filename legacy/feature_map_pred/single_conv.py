...
if __name__ == '__main__':
    if os.path.exists('single_conv_log.txt'):
        os.system('rm single_conv_log.txt')
    if not os.path.isdir('single_conv_models'):
        os.mkdir('single_conv_models')
    np.random.seed(0)
    ...
    for ooo in range(100):
        for idx in idx_list:
            ...
            if cnt % 100 == 0:
                torch.save(model.state_dict(), os.path.join('single_conv_models', 'epoch%d.dat' % cnt))
