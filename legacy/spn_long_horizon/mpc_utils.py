...
class MPCBuffer(object):
    ...
    def sample_done(self, idx):
        if idx - self.args.frame_history_len * self.args.horizon < 0 or idx + 1 + self.args.pred_step >= self.num_in_buffer:
            return False
        else:
            done_list = self.done[idx - self.args.frame_history_len * self.args.horizon: idx + 1 + self.args.pred_step]
            if np.sum(done_list) > 0.0:
                return False
            else:
                return True
    ...
    def _encode_observation(self, idx):
        ...
        start_idx = end_idx - self.args.frame_history_len * self.args.horizon
        return np.concatenate([self.obs[i] for i in range(start_idx, end_idx, self.args.horizon)], axis=0)
        ...
        for idx in range(start_idx, end_idx - 1):
            if self.done[idx % self.args.buffer_size]:
                start_idx = idx + 1
        missing_context = self.args.frame_history_len * self.args.horizon - (end_idx - start_idx)
        # if zero padding is needed for missing context
        # or we are on the boundry of the buffer
        if start_idx < 0 or missing_context > 0:
            frames = [np.zeros_like(self.obs[0]) for _ in range(missing_context)]
            for idx in range(start_idx, end_idx, self.args.horizon):
