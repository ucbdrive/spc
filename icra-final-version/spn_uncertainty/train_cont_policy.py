...
from uncertainty_buffer import uncertain_buffer
import pdb
...
def train_policy(args, env, num_steps=40000000):
    ...
    uc_buffer = uncertain_buffer(args)
    ...
    for tt in range(num_imgs_start, num_steps):
        ...
        states = net(imgs=obs_var, function='predict_info')
        prev_info = copy.deepcopy(info)
        if not no_explore and uc_buffer.uncertainty(list(prev_info['pos'][:2]), action, states['pos'], states['angle'], states['speed']) > args.uncertainty_threshold:
            action = np.random.uniform(-1, 1, size=(2,))
        # if num_episode % 3 == 0:
        #     action[1] = action[1]*-1.0
        obs, reward, done, info = env.step(action)
        uc_buffer.append_condition(list(prev_info['pos'][:2]), action, states['pos'], states['angle'], states['speed'])
        uc_buffer.append_gt(info['trackPos'], info['angle'], info['speed'])
        if 'carla' in args.env:
            obs, seg = obs
