...
class ConvLSTMNet(nn.Module):
    ...
    def predict_info(self, x):
        output_dict = dict()
        with torch.no_grad():
            _, rx, _ = self.get_feature(x)
            output_dict['pos'] = self.pos_layer(rx[:, -self.args.classes:, :, :])
            assert output_dict['pos'].size() == torch.Size([1, 1])
            output_dict['pos'] = output_dict['pos'][0, 0]

            output_dict['angle'] = self.angle_layer(rx[:, -self.args.classes:, :, :])
            assert output_dict['angle'].size() == torch.Size([1, 1])
            output_dict['angle'] = output_dict['angle'][0, 0]

            output_dict['speed'] = self.speed_layer(rx)
            assert output_dict['speed'].size() == torch.Size([1, 1])
            output_dict['speed'] = output_dict['speed'][0, 0]
        return output_dict
...
class ConvLSTMMulti(nn.Module):
    ...
    def forward(self, imgs, actions=None, hidden=None, cell=None, get_feature=False, training=True, function='', action_var=None):
        if function == 'predict_info':
            return self.conv_lstm.predict_info(imgs)
        if get_feature:
