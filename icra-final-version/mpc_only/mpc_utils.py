...
class MPCBuffer(object):
    ...
    def sample_collision(self, batch_size):
        ...
        if self.args.one_hot:
            feature = Variable(torch.from_numpy(np.concatenate([OneHotEncoder(n_values=self.args.classes, sparse=False).fit_transform(self.seg[idx, 0]).reshape(self.args.frame_height, self.args.frame_width, 1, self.args.classes).transpose(2, 3, 0, 1) for idx in idxes], 0)).float(), requires_grad=False)
        else:
    ...
    def sample_distance(self, batch_size):
        ...
        while len(idxes) < batch_size:
            ...
            if idx not in idxes and np.sum(self.done[idx-self.args.frame_history_len+1:idx+1]) == 0:
                idxes.append(idx)
        if self.args.one_hot:
            feature = Variable(torch.from_numpy(np.concatenate([OneHotEncoder(n_values=self.args.classes, sparse=False).fit_transform(self.seg[(idx-self.args.frame_history_len+1):(idx+1), 0].reshape(self.args.frame_history_len, self.args.frame_height * self.args.frame_width)).reshape(self.args.frame_history_len, args.frame_height, args.frame_width, 1, self.args.classes).transpose(3, 0, 4, 1, 2).reshape(1, self.args.frame_history_len * self.args.classes, args.frame_height, args.frame_width) for idx in idxes], 0)).float(), requires_grad=False)
        else:
            feature = Variable(torch.from_numpy(np.concatenate([self.seg[(idx-self.args.frame_history_len+1):(idx+1)].reshape(1, self.args.frame_history_len, self.args.frame_height, self.args.frame_width) for idx in idxes], 0)).float(), requires_grad=False)
    ...
    def sample_seq(self):
        ...
        while np.sum(self.done[idx-self.args.frame_history_len+1:idx+21]) > 0:
            idx = random.randint(0, self.num_in_buffer - 2)
        feature = Variable(torch.from_numpy(np.concatenate([OneHotEncoder(n_values=self.args.classes, sparse=False).fit_transform(self.seg[idx+i, 0]).reshape(self.args.frame_height, self.args.frame_width, 1, self.args.classes).transpose(2, 3, 0, 1) for i in range(1-self.args.frame_history_len, 1)], 0).reshape(1, self.args.frame_history_len * self.args.classes, self.args.frame_height, self.args.frame_width)).float(), requires_grad=False)
        action = Variable(torch.from_numpy(self.action[idx: idx+20][np.newaxis, :]), requires_grad=False)
        seg = Variable(torch.from_numpy(self.seg[idx+1: idx+21].reshape(20, self.args.frame_height, self.args.frame_width)).long(), requires_grad=False)
        signals = dict()
    ...
    def sample_fcn(self, batch_size):
        ...
        while len(idxes) < batch_size:
            ...
            if idx not in idxes and np.sum(self.done[idx-self.args.frame_history_len+1:idx+1]) == 0:
                idxes.append(idx)
        if self.args.one_hot:
            feature = Variable(torch.from_numpy(np.concatenate([OneHotEncoder(n_values=self.args.classes, sparse=False).fit_transform(self.seg[(idx-self.args.frame_history_len+1):(idx+1), 0].reshape(self.args.frame_height * self.args.frame_history_len, self.args.frame_width)).reshape(self.args.frame_history_len, self.args.frame_height, self.args.frame_width, 1, self.args.classes).transpose(3, 0, 4, 1, 2).reshape(1, self.args.frame_history_len * self.args.classes, self.args.frame_height, self.args.frame_width) for idx in idxes], 0)).float(), requires_grad=False)
        else:
            feature = Variable(torch.from_numpy(np.concatenate([self.seg[(idx-self.args.frame_history_len+1):(idx+1)].reshape(1, self.args.frame_history_len, self.args.frame_height, self.args.frame_width) for idx in idxes], 0)).float(), requires_grad=False)
    ...
    def sample_done(self, idx):
        if idx < 10 or idx >= self.num_in_buffer - self.args.pred_step - 10:
            ...
            done_list = self.done[idx - self.args.frame_history_len: idx + 1 + self.args.pred_step]
    ...
    def _encode_sample(self, idxes):
        ...
        if self.args.use_seg:
            data_dict['seg_batch'] = np.concatenate([self.seg[idx+1: idx + self.args.pred_step + 1, :][np.newaxis, :] for idx in idxes], 0)
    ...
    def store_frame(self, frame):
        if self.obs is None:
            self.obs = np.empty([self.args.buffer_size] + list(frame.shape), dtype=np.float32)
            ...
            if self.args.use_offroad:
                self.offroad = np.empty([self.args.buffer_size, 1], dtype=np.uint8)  # if 'torcs' in self.args.env else np.float32)
            if self.args.use_otherlane:
