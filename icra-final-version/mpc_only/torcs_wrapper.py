...
class TorcsWrapper:
    def __init__(self, seg_model, env, imsize=(256, 256), random_reset=True, continuous=True):
        self.env = env
        self.imsize = imsize
    ...
    def convert_seg(self, obs):
        ...
        if torch.cuda.is_available():
            obs = obs.cuda()
        with torch.no_grad():
            xx, x, _ = self.seg_model(obs)
            seg = torch.argmax(x, dim=1)
        xx = np.squeeze(xx.data.cpu().numpy(), axis=0)
        seg = np.squeeze(seg.data.cpu().numpy(), axis=0)
        return xx, seg
    ...
    def reset(self, rand_reset=True, restart=False):
        ...
        self.coll_cnt = 0
        self.done_cnt = 0
        return self.convert_seg(obs), info
    ...
    def step(self, action):
        ...
        return self.convert_seg(obs), reward, done, info
