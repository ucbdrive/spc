...
def sample_cont_action(args, net, imgs, info=None, prev_action=None, testing=False, avg_img=0, std_img=1.0, tt=0, action_var=None):
    ...
    if args.sample_based_planning:
        imgs = imgs.repeat(2 ** args.pred_step, 1, 1, 1, 1)
        action_var = action_var.repeat(2 ** (args.pred_step - 2), 1, 1)
        this_action0 = torch.zeros(2 ** args.pred_step, args.pred_step, 1)
        for i in range(2 ** args.pred_step):
            for j in range(args.pred_step):
                this_action0[i, j, 0] = int(bool(i & (1 << j)))
        this_action = Variable(this_action0.cuda().float(), requires_grad=False)
        loss = np.zeros(2 ** args.pred_step)
...
def tile(action, num_total_act):
    return action.view(action.size(0), num_total_act, 1, 1).repeat(1, 1, 64, 32)
