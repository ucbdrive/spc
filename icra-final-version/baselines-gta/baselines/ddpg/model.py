...
import baselines.ddpg.drn as drn
import baselines.ddpg.dla as dla
from baselines.ddpg.dla_up import DLAUp, Identity, fill_up_weights, BatchNorm
import baselines.ddpg.dla_up as dla_up
import numpy as np
import math
import pdb
from baselines.ddpg.end_layer import end_layer
from baselines.ddpg.conv_lstm import convLSTM
from baselines.ddpg.fcn import fcn
from baselines.ddpg.utils import weights_init, tile, tile_first
from baselines.ddpg.convLSTM2 import convLSTM2
    ...
    def forward(self, x, action, with_encode=False, hidden=None, cell=None, training=True, action_var=None):
        if not with_encode:
            if self.args.use_seg:
                ...
                x = rx = self.get_feature(x)

        # else:
        #     x = x.detach()
        if hidden is None or cell is None:
            if not self.args.use_seg:
                hidden = rx  # Variable(torch.zeros(shape))
                cell = rx
            else:
                if self.args.use_lstm:
                    cell = Variable(torch.zeros(rx.size(0), self.args.classes, args.frame_height, args.frame_width), requires_grad=False)  # Variable(torch.zeros(shape))
        ...
        if self.args.use_seg:
            ...
            nx_feature_enc = rx = hidden  # .view(-1, self.args.hidden_dim)
...
class ConvLSTMMulti(nn.Module):
    ...
    def get_feature(self, x, next_obs=False):
        ...
        batch_size = x.size(0)
        if next_obs:
            return self.conv_lstm.get_feature(x.view(batch_size*self.args.pred_step, 3*self.args.frame_history_len, 256, 256)).view(batch_size, self.args.pred_step, self.args.hidden_dim)
        else:
            return self.conv_lstm.get_feature(x[:, 0, :, :, :])
    ...
    def predict_feature(self, x, action):
        ...
        hidden = x
        cell = Variable(torch.zeros(batch_size, self.args.classes, args.frame_height, args.frame_width), requires_grad=False)
        if torch.cuda.is_available():
            cell = cell.cuda()
        result = []
    ...
    def forward(self, imgs, actions=None, hidden=None, cell=None, get_feature=False, training=True, function='', action_var=None, next_obs=False):
        ...
        if get_feature:
            return self.get_feature(imgs, next_obs)
		...
		"""
        _, seg, _ = self.conv_lstm.drnseg(imgs)	
        return seg
