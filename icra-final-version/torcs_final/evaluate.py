...
class ActionSampleManager:
    ...
    def sample_action(self, net, dqn_net, obs, obs_var, action_var, avg_img, std_img, info, get_first=True):
        if False:
            return self.process_act(self.prev_act, self.prev_dqn_act)
        else:
            dqn_act = None
            if self.args.continuous:
                action = sample_cont_action(self.args, net, obs_var, info=info, prev_action=np.array([0.5, 0.01]), avg_img=avg_img, std_img=std_img, tt=0, action_var=action_var, get_first=get_first)
...
def evaluate_policy(args, env):
    ...
    if args.visualize:
        ...
        for i in range(steps):
            ...
            _action, dqn_action = action_manager.sample_action(net, None, obs, obs_var, action_var, avg_img, std_img, info, get_first=False)
            action = actions[i]
            if True:  # i in stops:
