...
import sedla_up as dlaup
from utils import draw_from_pred_gta
...
class GTAWrapper:
    def __init__(self, env, imsize=(256, 256), random_reset = True, continuous = True):
        ...
        self.observation_space = Box(low=0, high=1, shape=(256, 256, 32), dtype=np.float32)
    ...
    def get_segmentation(self):
        ...
        seg = simplify(seg)
        # cv2.imwrite('vis/seg%s.png' % self.epi_len, draw_from_pred_gta(seg))
        # print('seg return')
        return seg
    ...
    def step(self, action):
        ...
        this_action[1] = 0
        # this_action[0] = max(real_action[0], 0) #*0.5 + 0.5
        # this_action[1] = max(-real_action[0], 0)
