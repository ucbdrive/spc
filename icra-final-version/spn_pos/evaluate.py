...
def evaluate_policy(args, env):
    ...
    if args.visualize:
        ...
        if not os.path.isdir('demo'):
            os.mkdir('demo')
        if not os.path.isdir(os.path.join('demo', str(steps))):
            os.mkdir(os.path.join('demo', str(steps)))
        cv2.putText(raw_obs, 'Nect Action: [%0.1f, %0.1f]' % (action[0], action[1]), (70, 400), cv2.FONT_HERSHEY_DUPLEX, 1.2, (255, 255, 0), 2)
        cv2.imwrite(os.path.join('demo', str(steps), 'obs.png'), cv2.cvtColor(raw_obs, cv2.COLOR_BGR2RGB))

        action = torch.from_numpy(action).view(1, 1, 2).repeat(1, args.pred_step, 1)
        action = Variable(action.cuda().float(), requires_grad=False)
        obs_var = buffer_manager.store_frame(obs)
        if args.normalize:
            obs_var = (obs_var.contiguous() - avg_img) / (std_img)
        else:
        ...
        for i in range(args.pred_step):
            ...
            cv2.imwrite(os.path.join('demo', str(steps), 'seg%d.png' % (i+1)), cv2.cvtColor(img, cv2.COLOR_BGR2RGB))
