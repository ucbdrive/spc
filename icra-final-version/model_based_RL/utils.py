...
def train_model(args, train_net, mpc_buffer, epoch, avg_img_t, std_img_t):
    ...
    if args.use_seg:
        target['nx_obs_batch'] = target['nx_obs_batch'][:, :, -3:, ...]
    ...
    if args.use_collision:
        ...
        coll_ls = nn.CrossEntropyLoss(reduce=False)(output['coll_prob'].view(-1, 2), target['coll_batch'].view(-1).long())
        coll_ls = torch.sum(coll_ls.view(-1, args.pred_step, 1) * weight) / args.batch_size
        loss += coll_ls
    ...
    if args.use_offroad:
        ...
        offroad_ls = nn.CrossEntropyLoss(reduce=False)(output['offroad_prob'].view(-1, 2), target['off_batch'].view(-1).long())
        offroad_ls = torch.sum(offroad_ls.view(-1, args.pred_step, 1) * weight) / args.batch_size
        loss += offroad_ls
    ...
    if args.use_otherlane:
        ...
        loss += otherlane_ls / args.batch_size
        print('otherlane ls', otherlane_ls.data.cpu().numpy())

    if args.use_distance:
        dist_ls = torch.sqrt(nn.MSELoss(reduce=False)(output['dist'].view(-1, args.pred_step), target['dist_batch'][:, 1:].view(-1, args.pred_step)))
        dist_ls = torch.sum(torch.sum(dist_ls, dim=0).view(-1, args.pred_step, 1) * weight) / args.batch_size * 0.01
        loss += dist_ls
        print('dist ls', dist_ls.data.cpu().numpy())

    # value_ls = nn.MSELoss(reduce=False)(output['value'].view(-1, args.pred_step), target['value_batch'].view(-1, args.pred_step))
    # value_ls = torch.sqrt(torch.sum(torch.sum(value_ls, dim=0).view(-1, args.pred_step, 1) * weight))
    # loss += 0.01 * value_ls
    ...
    if args.use_seg:
        ...
        if True:
            ...
            pred_ls = torch.sum(pred * (torch.log(pred) - torch.log(nximg_enc)), dim=-1)
        ...
        pred = output['seg_pred'][:, :, ...].contiguous().view(args.batch_size, -1)
        nximg_enc = nximg_enc.contiguous().view(args.batch_size, -1)
        pred_ls = torch.sqrt(nn.MSELoss()(pred, nximg_enc))
    loss += pred_ls
    print('pred ls', pred_ls.data.cpu().numpy())

    if args.use_pos:
        pos_loss = torch.sqrt((nn.MSELoss()(torch.exp(output['pos']/10), torch.exp(target['pos_batch'][:, :, :]/10)))) / args.batch_size
        loss += pos_loss
        print('pos ls', pos_loss.data.cpu().numpy())
    if args.use_angle:
        angle_loss = torch.sqrt((nn.MSELoss()(torch.exp(output['angle']), torch.exp(target['angle_batch'][:, 1:, :])))) / args.batch_size
        loss += angle_loss
        print('angle ls', angle_loss.data.cpu().numpy())
    if args.use_speed:
        speed_loss = torch.sqrt(nn.MSELoss()(output['speed'], target['sp_batch'][:, 1:, :])) / args.batch_size
        loss += speed_loss
...
def visualize(args, target, output):
    ...
    if False:  # args.use_seg:
        ...
        for i in range(args.pred_step):
            imsave('visualize/%d.png' % i, np.concatenate([cv2.cvtColor(observation[i], cv2.COLOR_BGR2RGB), draw_from_pred(prediction[i])], 1))
...
def get_action_loss(args, net, imgs, actions, action_var=None, target=None, hidden=None, cell=None, gpu=0):
    ...
    if args.sample_with_collision:
        output['coll_prob'] = F.softmax(output['coll_prob'].contiguous().view(-1, 2), dim=1).contiguous().view(batch_size, args.pred_step, 2)
        output['offroad_prob'] = F.softmax(output['offroad_prob'].contiguous().view(-1, 2), dim=1).contiguous().view(batch_size, args.pred_step, 2)
        if 'torcs' in args.env:
    ...
    # loss -= output['value'][:, -1, 0] * (args.gamma ** args.pred_step)
    return loss
