...
class ConvLSTMNet(nn.Module):
    def __init__(self, args):
        ...
        if args.one_hot:
            ...
            self.value_layer = end_layer(args, args.classes * args.frame_history_len, 1)
        else:
    ...
    def forward(self, x, action, with_encode=False, hidden=None, cell=None, training=True, action_var=None):
        ...
        output_dict['coll_prob'] = self.coll_layer(rx)
        output_dict['offroad_prob'] = self.off_layer(rx)
        output_dict['dist'] = self.dist_layer(hidden)
        # output_dict['value'] = self.value_layer(hidden)
...
class ConvLSTMMulti(nn.Module):
    ...
    def predict_feature(self, x, action):
        ...
        hidden = x
        cell = Variable(torch.zeros(batch_size, self.args.classes, self.args.frame_height, self.args.frame_width), requires_grad=False)
        if torch.cuda.is_available():
            cell = cell.cuda()
        result = []
