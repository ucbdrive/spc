...
class GlobalConvolutionV5(nn.Module):
    ...
    def __init__(self, c, o, lk, sk):
        ...
        self.proj = nn.Sequential(
            nn.Conv2d(c, o, kernel_size=sk, stride=1,
                      padding=sk // 2, bias=False),
            BatchNorm(o),
            nn.ReLU(inplace=True))
...
class GlobalConvolutionV7(nn.Module):

    def __init__(self, c, o, lk, sk):
        super(GlobalConvolutionV7, self).__init__()
        self.gcl = nn.Sequential(
            nn.Conv2d(c, o, kernel_size=(lk, 1), padding=(lk // 2, 0), stride=1),
            nn.Conv2d(o, o, kernel_size=(1, lk), padding=(0, lk // 2), stride=1))
        self.gcr = nn.Sequential(
...
class IDAUpV7(nn.Module):

    def __init__(self, large_kernel, small_kernel, out_dim, channels, up_factors):
        super(IDAUpV7, self).__init__()
        self.channels = channels
        self.out_dim = out_dim
        for i, c in enumerate(channels):
            if c == out_dim:
                ...
                proj = GlobalConvolutionV7(c, out_dim, large_kernel, small_kernel)
            f = int(up_factors[i])
            if f == 1:
                up = Identity()
            else:
...
class GlobalConvolutionV8(nn.Module):

    def __init__(self, c, o, lk, sk):
        super(GlobalConvolutionV8, self).__init__()
        self.gcl = nn.Sequential(
            nn.Conv2d(c, o, kernel_size=(lk, 1), padding=(lk // 2, 0), stride=1),
            nn.Conv2d(o, o, kernel_size=(1, lk), padding=(0, lk // 2), stride=1))
        self.gcr = nn.Sequential(
...
class IDAUpV8(nn.Module):

    def __init__(self, large_kernel, small_kernel, out_dim, channels, up_factors):
        super(IDAUpV8, self).__init__()
        self.channels = channels
        self.out_dim = out_dim
        for i, c in enumerate(channels):
            if c == out_dim:
                ...
                proj = GlobalConvolutionV8(c, out_dim, large_kernel, small_kernel)
            f = int(up_factors[i])
            if f == 1:
                up = Identity()
            else:
...
class GlobalConvolutionV9(nn.Module):

    def __init__(self, c, o, lk, sk):
        super(GlobalConvolutionV9, self).__init__()
        self.gcl = nn.Sequential(
            nn.Conv2d(c, o, kernel_size=(lk, 1), padding=(lk // 2, 0), stride=1),
            nn.Conv2d(o, o, kernel_size=(1, lk), padding=(0, lk // 2), stride=1))
        self.gcr = nn.Sequential(
...
class IDAUpV9(nn.Module):

    def __init__(self, large_kernel, small_kernel, out_dim, channels, up_factors):
        super(IDAUpV9, self).__init__()
        self.channels = channels
        self.out_dim = out_dim
        for i, c in enumerate(channels):
            if c == out_dim:
        ...
        for i in range(1, len(channels)):
            setattr(self, 'node_' + str(i), GlobalConvolutionV9(out_dim, out_dim, large_kernel, small_kernel))
...
class GlobalConvolutionV10(nn.Module):

    def __init__(self, c, o, lk, sk):
        super(GlobalConvolutionV10, self).__init__()
        self.gcl = nn.Sequential(
            nn.Conv2d(c, o, kernel_size=(lk, 1), padding=(lk // 2, 0), stride=1),
            nn.Conv2d(o, o, kernel_size=(1, lk), padding=(0, lk // 2), stride=1))
        self.gcr = nn.Sequential(
...
class IDAUpV10(nn.Module):

    def __init__(self, large_kernel, small_kernel, out_dim, channels, up_factors):
        super(IDAUpV10, self).__init__()
        self.channels = channels
        self.out_dim = out_dim
        for i, c in enumerate(channels):
            if c == out_dim:
        ...
        for i in range(1, len(channels)):
            setattr(self, 'node_' + str(i), GlobalConvolutionV10(out_dim, out_dim, large_kernel, small_kernel))
    ...
    def __init__(self, channels, scales=(1, 2, 4, 8, 16), in_channels=None, 
        ...
        for i in range(len(channels) - 1):
            ...
            if version == 'v1':
                ...
                setattr(self, 'ida_{}'.format(i),
                        IDAUpV7(large_kernel, small_kernel, channels[j], in_channels[j:],
                              scales[j:] // scales[j]))
            elif version == 'v8':
                setattr(self, 'ida_{}'.format(i),
                        IDAUpV8(large_kernel, small_kernel, channels[j], in_channels[j:],
                              scales[j:] // scales[j]))
            elif version == 'v9':
                setattr(self, 'ida_{}'.format(i),
                        IDAUpV9(large_kernel, small_kernel, channels[j], in_channels[j:],
                              scales[j:] // scales[j]))
            elif version == 'v10':
                setattr(self, 'ida_{}'.format(i),
                        IDAUpV10(large_kernel, small_kernel, channels[j], in_channels[j:],
                              scales[j:] // scales[j]))
            scales[j + 1:] = scales[j]
...
def dla34up_gc_bn_v7(classes, pretrained_base=None, **kwargs):
    return DLASeg('dla34', classes, pretrained_base=pretrained_base, version='v7', **kwargs)

def dla34up_gc_bn_v8(classes, pretrained_base=None, **kwargs):
    return DLASeg('dla34', classes, pretrained_base=pretrained_base, version='v8', **kwargs)

def dla34up_gc_bn_v9(classes, pretrained_base=None, **kwargs):
    return DLASeg('dla34', classes, pretrained_base=pretrained_base, version='v9', **kwargs)
