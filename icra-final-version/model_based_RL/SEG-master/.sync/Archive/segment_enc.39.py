...
def train(args, train_loader, model, criterion, optimizer, epoch, print_freq=10):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        # conf, pred = torch.max(output[0], 1)
        ...
        if i % print_freq == 0:
            print('Epoch: [{0}][{1}/{2}]\t'
                  ...
                  'Loss {loss.avg:.4f}'.format(
                      ...
                      data_time=data_time, loss=losses)) #score=torch.mean(per_class_iu(hist)) * 100))
        ...
        #           'Score {score:.2f}'.format(
        #               epoch, i, len(train_loader), batch_time=batch_time,
        #               data_time=data_time, loss=losses, score=round(np.nanmean(per_class_iu(hist)) * 100, 2))) #score=torch.mean(per_class_iu(hist)) * 100))
...
def train_seg(args):
    ...
    # criterion = encoding.nn.SegmentationLosses(se_loss=False, aux=False, nclass=args.classes, ignore_index=255)
...
#     out = torch.diag(hist) / (torch.sum(hist, 1) + torch.sum(hist, 0) - torch.diag(hist))
#     out[out != out] = 0 #https://discuss.pytorch.org/t/how-to-set-nan-in-tensor-to-0/3918/4
