...
class BufferManager:
    def __init__(self, args=None):
        ...
        self.prev_act = np.array([-1.0]) if args.continuous else 1
    ...
    def reset(self, step, log_name='log_train_torcs.txt'):
        ...
        self.rewards = 0
        self.prev_act = np.array([-1.0]) if self.args.continuous else 1
        ...
        if tt % self.args.num_same_step != 0:
            ...
            if self.args.continuous:
                if (random.random() <= 1 - exploration.value(tt) or no_explore) and not must_explore:
                    action = sample_cont_action(self.args, net, obs_var, info=info, prev_action=np.array([-1.0]), avg_img=avg_img, std_img=std_img, tt=tt, action_var=action_var)
                else:
                    action = np.random.randint(2, size=1) * 2 - 1
                if self.args.use_dqn:
...
def train_policy(args, env, num_steps=40000000):
    ...
    video = cv2.VideoWriter(os.path.join(video_folder, 'video.avi'), cv2.VideoWriter_fourcc(*'MJPG'), 10.0, (288, 512), True)

    done_cnt = 0
    _, info = env.reset()
    video.write(env.real_obs)
    ...
    must_explore = False
    print('start game')
    action_var = Variable(torch.from_numpy(np.array([-1.0])).repeat(1, args.frame_history_len - 1, 1), requires_grad=False)
    for tt in range(num_imgs_start, num_steps):
        ...
        seg = info['segmentation']
        print('Action = %0.2f, Reward = %d, Done = %s' % (float(action), reward, str(done)))
        ...
        if done:
            ...
            if not os.path.isdir(video_folder):
                os.makedirs(video_folder)
            video = cv2.VideoWriter(os.path.join(video_folder, 'video.avi'), cv2.VideoWriter_fourcc('M','J','P','G'), 10.0, (288, 512), True)

            no_explore = not no_explore
            done_cnt += 1
            obs, info = env.reset()
            obs, _, _, info = env.step(np.array([-1.0])) if args.continuous else env.step(1)
            seg = info['segmentation']
