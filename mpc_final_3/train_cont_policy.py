...
def train_policy(args, env, num_steps=40000000):
    ...
    obs = OneHotEncoder(n_values=3, sparse=False).fit_transform(env.env.get_segmentation()).reshape(256, 256, 3)
    ...
    for tt in range(num_imgs_start, num_steps):
        ...
        if done:
            ...
            if args.sample_with_pos:
                args.target_pos = np.random.uniform(-7, 7)
            if args.target_speed >0:
