...
def train_model(args, train_net, mpc_buffer, epoch, avg_img_t, std_img_t):
    ...
    if args.use_seg:
        ...
        target['seg_batch'] = target['nx_obs_batch'][:, :, -args.classes:, ...].max(2)[1].long()
...
def visualize(args, target, output):
    ...
    if args.use_seg:
        ...
        for i in range(pred.shape[0]):
            p_acc += pixel_accuracy(pred[i], gt[i])
            m_acc += mean_accuracy(pred[i], gt[i])
            m_IU += mean_IU(pred[i], gt[i])
            fw_IU += frequency_weighted_IU(pred[i], gt[i])
        p_acc /= pred.shape[0]
        m_acc /= pred.shape[0]
        m_IU /= pred.shape[0]
        fw_IU /= pred.shape[0]
